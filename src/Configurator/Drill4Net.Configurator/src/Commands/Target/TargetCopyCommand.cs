...
Position order: source config path, destination config path, version destination, target output directory. Positions are used on a residual basis if the corresponding named argument is not found.
